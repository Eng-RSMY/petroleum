<template>
  <div class="container">
    <div class="orderInfo">
      ...
      <p v-show="fromOrder">
        <span>预装单价：</span>
        <span>{{orderInfo.orderPrice==null?"暂无数据":orderInfo.orderPrice+" 元/吨"}}</span>
      </p>
      <p v-show="fromOrder">
        <span>结算单价：</span>
        <span>{{orderInfo.price==null?"暂无数据":orderInfo.price+" 元/吨"}}</span>
      </p>
      <p v-show="fromOrder">
        <span>订单总价：</span>
    ...
    <div class="weui-cell" style="border-top: none;padding: 20px 0px;">
      ...
      <div class="weui-cell__ft">
        <button class="weui-btn weui-btn_primary button" style="background-color:#6f99fc;" v-if="fromOrder" @click.stop="del()">编辑
        </button>
      </div>
    </div>
    ...
    <div class="keyNums" v-if="isInputData2Show">
      <div class="i-b-li lf" v-for="(keyItem, keyIndex) of inputData2" :key="keyIndex" @click="getNum(keyItem)">{{
        keyItem }}
      </div>
