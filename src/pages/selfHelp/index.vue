<template>
  <div class="container">
    ...
    <div class="product">
      ...
      <div class="weui-cell" style="border-top: none">
        ...
        <div class="weui-cell__ft">
          <picker @change="bindPickerChange" :value="index" :range="categories">
            <view class="picker">{{pickSelect}}</view>
          </picker>
        </div>
      </div>
      <div class="weui-cell">
        ...
        <div class="weui-cell__ft">
          ...
          <div class="ishave" v-else>
            <picker @change="bindPickerChange1" :value="index" :range="oils">
              <view class="picker">{{pickSelect1}}</view>
            </picker>
          </div>
        </div>
      </div>
      ...
      <div
        class="weui-cell"
        style="border-bottom: 1px solid #ddd;color:#a6a6a6;position: relative;"
      >
        <div class="weui-cell__bd">
          <p>物料单价</p>
        </div>
    ...
    <div class="choose">
      ...
      <button
        ...
        v-if="showDriver"
        @click="chooseDriver"
        style="background-color: #fff;border-color: #dedede"
      >请选择</button>
    ...
    <div class="input-box" v-if="isInputData1Show">
      <div
        class="i-b-li lf"
        v-for="(item, index) of inputData1"
        :key="index"
        @click="getPro(item)"
        style="float: left;"
      >{{ item }}</div>
    ...
    <div class="keyNums" v-if="isInputData2Show">
      <div
        class="i-b-li lf"
        v-for="(keyItem, keyIndex) of inputData2"
        :key="keyIndex"
        @click="getNum(keyItem)"
        style="float: left;"
      >{{ keyItem }}</div>
    ...
    <div class="js_dialog" id="iosDialog1" v-if="showDialog">
      ...
      <div class="weui-dialog">
        <div class="weui-dialog__hd">
          <strong class="weui-dialog__title">警告</strong>
        </div>
...
export default {
  ...
  methods: {
    bindPickerChange: function(e) {
      ...
      this.$http
        .get("/self_order/oils", { categoryName: categoryName })
        .then(res => {
          if (res.status == "200") {
            this.oils = [];
        ...
        this.$http
          ...
          .then(res => {
            if (res.status == "200") {
              ...
              wx.navigateTo({
                url:
                  "../../pages/order/orderInfo/main?orderInfo=" +
                  orderInfo +
                  "&from=selfHelp",
                fail: function(res) {}
              });
  ...
  onShow() {
    ...
    this.$http.get("/self_order/company_account").then(res => {
      if (res.status == "200") {
